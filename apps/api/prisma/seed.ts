...
const practiceAreas = [
  ...
  ['Property Law', 'Title checks, registration, tenancy, and property disputes.'],
...
async function main() {
    ...
    [
      ...
      await prisma.lawyer.upsert({
        ...
        create: {
          ...
          educations: {
            create: [{ degree: 'LL.B.', institute: 'National Law University', year: 2010 }],
          },
          experiences: {
            create: [{ title: 'Partner', institution: 'Independent Practice', years: lawyer.experience }],
          },
          availability: {
            create: [
  ...
  const firstLawyer = await prisma.lawyer.findUniqueOrThrow({ where: { userId: lawyerUsers[0].id } });
  await prisma.review.upsert({
    where: { customerId_lawyerId: { customerId: customer.id, lawyerId: firstLawyer.id } },
    update: {},
    create: {
